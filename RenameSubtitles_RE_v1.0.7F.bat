...
set "subext=ass;ssa;srt;idx;pgs;sup;smi;log;"
set "vidext=mp4;m4v;mkv;m2ts;avi;mpeg;mpg;txt;"
rem -------------------------------------------------------
...
set "echo_debug="
for /f "tokens=*" %%a in ('cmd /c echo') do (
	set "echo_mode=%%~a"
)
...
set "bat_ver=RenameSubtitles RE: Ver1.0.7 UTF-8"
...
set "subget_finish="
if exist "%ass_path%%s_path%" (
	call :check_subtitles "%ass_path%%s_path%"
	call :check_subget
...
setlocal DisableDelayedExpansion
if defined insert_v for /f "tokens=1,2 delims=%insert_v:~0,1%" %%a in ("%filter%") do (
	if not "%%~b"=="" (
...
setlocal DisableDelayedExpansion
if defined insert_s for /f "tokens=1,2 delims=%insert_s:~0,1%" %%a in ("%s_filter%") do (
	if not "%%~b"=="" (
