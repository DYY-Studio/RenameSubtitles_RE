...
echo [ 自动模式的运行速度与单核性能紧密相关          ]
echo [ 因而自动 非常慢！ 非常慢！ 非常慢！ 非常慢！  ]
echo [ 而且批量处理程序比较简单 分析出错率比较大     ]
echo [ 不是很懒的人非常非常非常建议手动输入匹配规则  ]
echo [ 预计时长 约2分钟完成13话分析 于 酷睿i5-4590   ]
...
for /f "skip=1 tokens=1* delims==" %%a in ('set %~1[') do (
...
if not defined ls_step (
	set "ls_step=5"
) else if %ls_step% LSS 1 set "ls_step=5"
set /a ls_step2=ls_step-1
:long_string_loop
set /a string_long=string_long+ls_step
set "ls_check=!string_in:~%string_long%!"
set "ls_check_last=!ls_check:~%ls_step2%,1!"
if not "%ls_check_last%"=="" (
...
) else for /f "tokens=1* delims==" %%a in ('set C_%~1') do set "list_count=%%~b"
...
for /f "tokens=1* delims==" %%a in ('set C_%~1 2^>nul') do (
	for /l %%c in (0,1,%%~b) do (
       set "%~1[%%~c%]="
	)
)
...
if not exist "%wtofile%" (
	...
	) else del /q "%USERPROFILE%\rforbat.log"
) else echo.>NUL
