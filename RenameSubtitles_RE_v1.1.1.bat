...
rem -------------------------------------------------------
rem [new_match_method]
rem 控制是否使用于v1.1加入的全新匹配方法
rem.
rem 该匹配方法不使用数列逐个匹配，而是直接用文件名上写的集数，对应到字幕上的集数
rem 如"[TEST] TEST - 01 [TEST].mp4"和"[TEST] TESTASS 01 [ASSTEST].ass"
rem 视频匹配位置应该为[14,2]，那么将会从这里截取出集数"01"，将集数套用到字幕匹配位置[15,2]
rem 程序会发现视频文件名中位于[14,2]的"01"和字幕文件名中位于[15,2]的"01"是一样的
rem 那么这个字幕就是和视频匹配的

set "new_match_method=0"

rem 可用值
rem 	{未定义}:禁用（使用V1.0旧式搜索算法）
rem 	{任意值}:启用（使用V1.1新式搜索算法）
rem 注
rem 	{未定义}即直接 set "new_match_method=" 不需要空格
rem -------------------------------------------------------
rem [direct_sub_match]
rem 控制是否直接丢弃字幕匹配规则中的 集数长度 参数
rem.
rem 在禁用该选项时，字幕搜索模块会先将匹配规则拆成两部分
rem 如："[TESTSUB] TEST - : [SUB].ass" 将拆成 "[TESTSUB] TEST - " 和 " [SUB].ass"两部分
rem 再插入集数，如："[TESTSUB] TEST - " & "01" & " [SUB].ass" 即 "[TESTSUB] TEST - 01 [SUB].ass"
rem.
rem 但在启用该选项时，字幕搜索模块将直接用通配符"*"代替整个后半部分
rem 如："[TESTSUB] TEST - " & "01" & "*" 即 "[TESTSUB] TEST - 01*"
rem.
rem 该选项也影响Auto模式，放弃搜索集数长度将会少量提升Auto的运行速度
rem 该选项只有在启用V1.1新式搜索的时候才会生效

set "direct_sub_match=0"
...
rem -------------------------------------------------------
rem [max_ep-range]
rem 控制在V1.1新匹配方法下的最大集数长度
rem 同时也影响auto模式下对集数末尾字符的搜索（搜索长度为 max_ep-range*2 )
...
if defined max_ep-range (
	if not %max_ep-range%0 GTR 0 (
		set "max_ep-range=5"
	)
) else set "max_ep-range=5"
...
set "bat_ver=RenameSubtitles RE: Ver1.1.1 UTF-8"
...
echo.
if not defined new_match_method (
	...
	echo [ 匹配规则即如何匹配 请用半角冒号代替集数位置   ]
	echo [ 您已启用新匹配算法 只需要标记集数位置即可     ]
	echo [ 例如[CASO][El_Cazador][GB_BIG5][01][DVDRIP]   ]
	echo [ 替换成[CASO][El_Cazador][GB_BIG5][:][DVDRIP]  ]
	echo [ 如果视频有非常麻烦的如校验值在文件名结尾      ]
	echo [ 请使用通配符 如[CASO][*][GB_BIG5][:][*        ]
...
if %stop_count% GTR 0 (
	echo [VINS] 在视频匹配规则中找到位置%instr[0]%，长度%get_loop%
	goto END_FIND_COUNT
)
if not "%a_char%"=="" (
...
) else if not "%get_count%"=="0" (
	if not "%a_char%"=="*" set "get_count=%a_char%"
	set "stop_count=1"
)
if %stop_count% GTR 0 (
    echo [VINS] 在视频匹配规则中找到位置%instr[0]%，结束于"%get_count%"字符
	goto END_FIND_COUNT
)
if not "%a_char%"=="" (
	set /a get_loop=get_loop+1
...
) else if defined s_instr[0] (
    set /a stop_count=stop_count+1
)
if %stop_count% GTR 0 (
	echo [SINS] 在字幕匹配规则中找到位置%s_instr[0]%，长度%get_count%
	goto END_FIND_SUB_COUNT
)
if not "%a_char%"=="" (
	set /a get_loop=get_loop+1
	goto FIND_SUB_COUNT
...
if not defined new_match_method goto old_loop_num


for /f "tokens=1* delims=:" %%a in ("%filter%") do (
	set "dir_filter=%%~a*%%~b*"
)
if defined vid_subdir (
	set "dir_set=/s /b"
) else set "dir_set=/b"
for /f "tokens=* delims=^|" %%a in ('dir %dir_set% /a:-D "%v_path%%dir_filter%"') do (
	for /f "tokens=1* delims==" %%b in ('set vidext[') do (
		if "%%~xa"==".%%~c" (
			echo "%%~a">"%vpfile%"
			call :get_epInfo "%%~nxa" "%vpath%"	
		)
	)
)
set "dir_set="
...
set "v_file=%~1"
setlocal EnableDelayedExpansion
set "epCache=!v_file:~%instr[0]%!"
set "epInfo="
set "dont_loop="
for /l %%a in (1,1,%max_ep-range%) do (
    call :get_epInfo_loop
)
set "epInfo=%epInfo%"
setlocal DisableDelayedExpansion
if defined epInfo (
	set "epInfo_check="
	set "epCache="
	echo [LOOP] 尝试匹配剧集"%epInfo%"
	call :SEARCH_ASS "%sub_path%" "%s_filter%" "%epInfo%"
)
goto :EOF
:get_epInfo_loop
if defined dont_loop goto :EOF
if "%epCache:~0,1%"=="%instr[1]%" (
	set "dont_loop=1"
	goto :EOF
)
set "epInfo=%epInfo%%epCache:~0,1%"
set "epCache=%epCache:~1%"
goto :EOF
...
setlocal EnableDelayedExpansion
if defined new_match_method goto end_s_add_zero
...
set "s_path_1="
if defined direct_sub_match if defined new_match_method (
	set "s_path=%s_path[0]%%sub_num%*"
) else set "s_path=%s_path[0]%%sub_num%%s_path[1]%"

set "subget_finish="
set "get_sub=0"
if not defined new_match_method (
	if not "%s_instr[1]%"=="1" set "s_path_1=%s_path[0]%%sub_num_r%%s_path[1]%"
) else if not defined direct_sub_match set "s_path_1=%s_path[0]%%sub_num%*"
...
if %sub_long%0 LEQ %video_long%0 goto :EOF
...
) else call :compare_list "c_v" "%count_range%" "1"
call :list_appear_mostV2 "comp_diff"

if not defined new_match_method (
	echo [CHKN] 容差值计算 a=±%deviation_a%,b=±%deviation_b%
) else (
	echo [CHKN] 容差值计算 a=±%deviation_a%
)

for /f "tokens=1* delims==" %%a in ('set comp_diff%l_largest:~12%') do (
	for /f "tokens=1,2 delims=," %%c in ("%%~b") do (
		echo [CHKN] a范围: %%~c ± %deviation_a%
		if not defined new_match_method echo [CHKN] b范围: %%~d ± %deviation_b%
...
set "vend_char="
if not defined new_match_method goto tv.maxN
echo [MAXN] 求最频繁出现的结束字符
call :list_appear_mostV2 "comp_diff_2"
echo [MAXN] 最频繁出现的结束字符为"%appear_most%"
set "vend_char=%appear_most%"
goto tv.maxN.end
:tv.maxN
echo [MAXN] 差异量求最大值
...
if defined ext_use_most_v (
	set "filter=%filter%%vend_char%*%vf_x%"
) else set "filter=%filter%%vend_char%*"
...
ping -n 3 -w 1000 127.0.0.1 1>NUL 2>NUL
goto :EOF
	...
	for /f "tokens=1,2 delims=," %%c in ("%%~b") do (
		for /f "tokens=1* delims==" %%e in ('set comp_diff') do (
			for /f "tokens=1,2 delims=," %%g in ("%%~f") do (
				...
				if not defined direct_sub_match (
					call :check_num "%%~h" "%%~d,%deviation_b%" "%%~e" "c_s"
				) else if not defined new_match_method (
					call :check_num "%%~h" "%%~d,%deviation_b%" "%%~e" "c_s"
				)
			)
		)
...
if not defined new_match_method (
    goto ts.maxN
) else if not defined direct_sub_match (
    goto ts.maxN
)
set "sf_c=1"
goto ts.maxN.end
...
rem call :compare_list "[list_name]" "[select_compare]" "[new_method_adapt]"
rem select_compare: 
...
for /l %%a in (0,1,%list_count%) do (
	if defined %~1[%%~a] for /f "tokens=1* delims==" %%b in ('set %~1[%%~a]') do (
		...
		call :compare_string "%%~c" "%~1" "%selcount%" "%~3"
	)
)

if defined drop_minus if not defined new_match_method (
	if not defined compare_limit_method (
		for /f "tokens=1* delims==" %%a in ('set comp_diff[') do (
			for /f "tokens=1,2 delims=," %%c in ("%%~b") do (
				if not %%~d GTR 0 (
					echo [DROP] 已舍弃数据"%%~a": "%%~b"
...
rem call :compare_string "[string]" "[compare_list]" "[selcount]" "[new_method_adapt]"
:compare_string
...
for /f "tokens=1* delims==" %%a in ('set num_appear%l_largest:~12%') do (
	if defined str_diff_end[0] (
		call :compare_string.str_diff_end "%%~b"
	) else (
...
if not "%comp_1%"=="%comp_2%" (
	...
	call :compare_char_range "%~1" "%~3"
	goto list_compare_diff
)
goto compare_char_loop
:list_compare_diff
call :time_calculation "%char_time%" "%time%" "char_speed"
rem if defined diff_end set /a diff_in=diff_in+1
if defined diff_count if defined diff_in (
	if defined diff_end (
		echo - 发现差异开始于"%diff_in%"，以"%diff_end%"结束 [%char_speed%]
		call :number_list "str_diff" "%diff_in%,%diff_end%"
	) else (
...
set "com_char=0"
set "has_diff="
set /a search_l=%max_ep-range%*2
if not "%~2"=="" (
	set "diff_end="
	goto compare_char_l2_newm
)
:compare_char_l2
...
if not "%com_char_1%"=="%com_char_2%" (
	if defined new_match_method if defined direct_sub_match (
		set "diff_count=X"
		goto :EOF
	)
	set "has_diff=0"
...
:compare_char_l2_newm
set "com_char_1=!in_comp:~%ccl_count%,1!"
set "com_char_2=!in_comp2:~%ccl_count%,1!"
if "%com_char_1%"=="" goto :EOF
if "%com_char_2%"=="" goto :EOF
rem echo "%com_char_1%" ^| "%com_char_2%" ^( "%diff_str_c1%" ^| "%diff_str_c2%" ^)
if defined has_diff (
	if "%com_char_1%"=="%com_char_2%" (
		call :non_numerical "%com_char1%"
		if not defined non_numerical (
			call :non_numerical "%com_char2%"
			if not defined non_numerical set "diff_end=%com_char_1%"
		)
		goto :EOF
    )
)
if defined has_diff (
	if defined diff_str_c1 if not defined diff_end (
		for /f "tokens=1,2 delims=%com_char_2%" %%a in ("A%diff_str_c1%A") do (
			if not "%%~b"=="" (
				call :non_numerical "%com_char_2%"
				if not defined non_numerical set "diff_end=%com_char_2%"
			)
		)
	)
	if defined diff_str_c2 if not defined diff_end (
		for /f "tokens=1,2 delims=%com_char_1%" %%a in ("A%diff_str_c2%A") do (
			if not "%%~b"=="" (
				call :non_numerical "%com_char_1%"
				if not defined non_numerical set "diff_end=%com_char_1%"
			)
		)
    )
) else if not "%com_char_1%"=="%com_char_2%" (
	set "has_diff=0"
	if not defined diff_str_c1 (
		set "diff_str_c1=!in_comp:~%ccl_count%,%search_l%!"
	)
	if not defined diff_str_c2 (
		set "diff_str_c2=!in_comp2:~%ccl_count%,%search_l%!"
	)
	set /a diff_count=diff_count+1
) else set /a diff_in=diff_in+1
if defined diff_end goto :EOF
set /a ccl_count=ccl_count+1
goto compare_char_l2_newm
...
rem call :non_numerical "[string] OR [char]"
:non_numerical
set "non_numerical="
set "numInput=%~1"
if "%numInput%"=="" goto :EOF
if "%numInput:~1%"=="" (
    call :non_numerical.single "0"
	goto :EOF
)
call :long_string "%numInput%" "inputLong" "0"
for /l %%a in (0,1,%inputLong%) do (
	if not defined non_numerical call :non_numerical.single "%~1"
)
goto :EOF
:non_numerical.single
set "numSingle=!numInput:~%~1,1!"
for /l %%a in (0,1,9) do (
	if not defined non_numerical if "%numSingle%"=="%%~a" (
		set "non_numerical=0"
	)
)
goto :EOF
