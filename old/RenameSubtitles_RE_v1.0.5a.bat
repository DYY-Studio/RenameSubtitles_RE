...
:norem_sub
echo [DELN] 正在尝试去除影响运行的半角感叹号（如果存在）
call :rename_file_nos "%v_path%"
if not "%v_path%"=="%sub_path%" call :rename_file_nos "%sub_path%"

if defined auto goto AUTO
goto AUTO_BACK

rem 该模块被丢弃了，因为各种兼容太麻烦
echo.
...
rem call :del_char_nos "[string]" "[char]"
rem [RETURN] dcn_in
:del_char_nos
if "%~1"=="" goto :EOF
set "dcn_in=%~1"
...
:del_char_nos_loop
for /f "tokens=1* delims=%dcn_cr%" %%a in ("%dcn_in%") do (
	if not "%%~b"=="" (
		call :del_char_nos_set "%%~a" "%%~b"
	) else (
		goto :EOF
	)
)
goto del_char_nos_loop
:del_char_nos_set
set "dcn_in=%~1%~2"
goto :EOF

rem call :rename_file_nos "[path]" "[enableDE]"
:rename_file_nos
setlocal DisableDelayedExpansion
if "%~1"=="" goto :EOF
for /r "%~1" %%a in ("*!*") do (
	call :rename_file_nos_work "%%~a"
)
if not "%~2"=="" setlocal EnableDelayedExpansion
goto :EOF
:rename_file_nos_work
if not exist "%~1" goto :EOF
call :del_char_nos "%~n1" "!"
rename "%~1" "%dcn_in%%~x1"
goto :EOF
