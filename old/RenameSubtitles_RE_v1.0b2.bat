...
if exist "%v_path%" (
	for /f "tokens=*" %%a in ("%v_path%") do set "chk_vpath=%%~aa"
) else (
	cls
	echo [ERRV] 找不到视频目录
	goto MAIN
...
if not defined sub_path set "sub_path=%v_path%"
if exist "%sub_path%" (
...
rem call :SEARCH_ASS "[v_path]" "[filter]" "[ep_num]" "[sub-dir][y/n] [unset]"
:SEARCH_ASS
set "ass_path=%~1"
if not "%~2"=="" ( 
