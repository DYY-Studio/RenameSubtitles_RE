...
rem [count_range]
rem 用于控制compare_list时进行比较的项数，可以加快AUTO，但错误率增大
set "count_range=15"
rem 可用值 {0~n(非负整数)} ，{留空} 即不启用（全项目搜索）
rem [count_range]从0开始计数，即=15时会比较16项
rem 推荐值10~20，不建议少于10。对于集数超过100的，必须使用大于或等于100的值以防止出错。

set "bat_ver=RenameSubtitles RE: Ver1.0.2 GB2312"
...
set "string_in=%~1"
:long_string_loop
set /a string_long=string_long+5
