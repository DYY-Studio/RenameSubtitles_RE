...
set "bat_ver=RenameSubtitles RE: Ver1.0.5(b) GB2312"
...
if %errorlevel%==1 (
	...
	if exist "%logfile%" move /-y "%logfile%" "%sub_path%rename_log.log"
)
if exist "%prefile%" del /q "%prefile%"
if exist "%vpfile%" del /q "%vpfile%"
...
rem call :Module_ReplaceBatch "[string]" "[replace_char]" "[to_char]" "[SPinsert]" "[enableDE]"
...
:RB.end_clear
if not "%~5"=="" setlocal EnableDelayedExpansion
set "RB_cache="
set "input_string="
set "RB.return="
set "output_string="
